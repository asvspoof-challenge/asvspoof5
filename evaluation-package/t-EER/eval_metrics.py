...
def process_ASVspoof5_score_files(cm_score_file, asv_score_file,label_file):
    ...
    trial_data = np.genfromtxt(label_file, dtype=str)
    trial_type = trial_data[:,1]
   
    # need to check here that the utt_ids are the same in cm and asv files
    X_tar = np.array([asv_scores[trial_type == 'target'], cm_scores[trial_type == 'target']],dtype=object)
    X_non = np.array([asv_scores[trial_type == 'nontarget'], cm_scores[trial_type == 'nontarget']],dtype=object)
    X_spf = np.array([asv_scores[trial_type == 'spoof'], cm_scores[trial_type == 'spoof']],dtype=object)
...
def performance(bona_cm,spoof_cm, Pfa_asv, Pmiss_asv, Pfa_spoof_asv, invert=False):
